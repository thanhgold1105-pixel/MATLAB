...
P_r_dB = 10;                   % power at R
P_r = 10^(P_r_dB / 10);
beta = 0.5;                    % fixed SI suppression

alpha_range = linspace(0, 1, 200);
P_s_dB_values = [5, 15, 25];   % 3 curves for sender power
lineStyles = {':', '--', '-.'};
...
% Loop over 3 P_s values
for p = 1:length(P_s_dB_values)
    P_s_dB = P_s_dB_values(p);
    P_s = 10^(P_s_dB / 10);
