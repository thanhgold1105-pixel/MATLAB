...
P_MD = zeros(length(Pc_dB), length(Pr_dB));
...
for idx = 1:length(Pc_dB)
    ...
    for i = 1:length(Pr_dB)
        ...
        % Aggregate over alpha — choose min, mean, or max
        P_MD_max = squeeze(max(P_MD, [], 3));  % size: (length(P_c), length(P_r))

        eta_max(idx, i) = max(real(eta_alpha));  % store max throughput for this Pr

         % Calculate max_information for fixed throughput value
        indices = find(abs(eta_max - fixed_throughput) < 1);
        if ~isempty(indices)
    ...
    plot(Pr_dB, detection_over_P_r(idx), 'k', ...
