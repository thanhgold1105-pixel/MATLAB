...
for idx = 1:length(P_avg_dB_values)
    ...
    eta_loss = zeros(length(fixed_throughput_values), length(max_eta_total));
    for j = 1:length(fixed_throughput_values)
        eta_loss(j, :) = max_eta_total - fixed_throughput_values(j);
    end
    eta_loss(eta_loss < 0) = 0; % Ensure eta_loss is non-negative

    % Average eta_loss across alpha values
    mean_eta_loss = mean(eta_loss, 2);

    % Plot eta_loss versus fixed_throughput_values
    plot(fixed_throughput_values, mean_eta_loss, lineStyles{idx},'Color','k' ,'LineWidth', 1.5);
end
...
%title('Mean Eta Loss vs. Fixed Throughput Values');
grid on;
hold off;
