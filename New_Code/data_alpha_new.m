...
for p = 1:length(P_avg_dB_values)
    ...
    % Loop over alpha_range to calculate the maximum eta_total for each alpha
    for i = 1:length(alpha_range)
        ...
        for k = 1:length(delta_range)
            ...
            if n_remaining <= 0 || beta <= 0 || P_c_value <= 0 || sigma_gc * P_r <= beta * sigma_hc * P_c_value
                eta_total_values_for_delta(k) = NaN;
                continue;
            end

            % % Calculate lambda and P_d
            % lambda = gammaincinv(1 - rho, n_value, 'lower');  % Using icdf for gammaincinv
            ...
            % Store eta_total for the current delta
            eta_total_values_for_delta(k) = eta_prime;
        end

        % Find the maximum eta_total for the current alpha across all delta values
        max_eta_total_per_alpha(i) = max(eta_total_values_for_delta, [], 'omitnan');
    end

    % Plot max eta_total versus alpha for the current P_avg_dB with line style and black color
    plot(alpha_range, max_eta_total_per_alpha, 'LineWidth', 1.5, 'Color', color, 'LineStyle', lineStyles{p});
